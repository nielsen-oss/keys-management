...
from .log_messages_consts import DEFINE_KEY_LOG_FORMAT, SUCCESS_DEFINE_KEY_LOG_FORMAT, GET_KEY_INFO_FORMAT, GET_KEY_DEBUG_FORMAT, \
    RV_KEY_LOG_FORMAT, KEY_CHANGED_INFO_FORMAT, \
    KEY_CHANGED_DEBUG_FORMAT, \
    REGISTER_ON_CHANGE_LOG_FORMAT, ON_HALT_LOG_FORMAT, ON_SKIP_LOG_FORMAT, CLEAN_KEYS_LOG_FORMAT, CLEAN_PREV_KEYS_LOG_FORMAT
from .errors import KeysManagementError, OnKeyChangedCallbackErrorStrategy, GetKeyError, KeyChangedError, KeyIsNotDefinedError, FetchAndSetStateFromRepoError, InvalidKeyStateError
...
class KeysManagement(object):
    def define_key(self, name: str, keys_store: KeysStore, is_stateless: bool, use_case: SecretKeyUseCase,
                   target_data_accessible: bool, keep_in_cache: bool, on_key_changed_callback_error_strategy: OnKeyChangedCallbackErrorStrategy = None) -> KeysManagement:
...
class KeysManagementImpl(KeysManagement):
    ...
    _callbacks_executions_error_handling: Dict[OnKeyChangedCallbackErrorStrategy, Callable]
    ...
    def define_key(self, name: str, keys_store: KeysStore, stateless: bool, use_case: SecretKeyUseCase,
                   target_data_accessible: bool, keep_in_cache: bool, on_key_changed_callback_error_strategy: OnKeyChangedCallbackErrorStrategy = None) -> KeysManagement:
        on_key_changed_callback_error_strategy = on_key_changed_callback_error_strategy if on_key_changed_callback_error_strategy is not None else OnKeyChangedCallbackErrorStrategy.HALT
    ...
    def _get_key_authentication_case(self, key_definition: SecretKeyDefinition,
        ...
        if purpose != SecretKeyUseCase.AUTHENTICATION:
            GetKeyError(key_name=key_definition.name, reason=PURPOSE_IS_NOT_AUTHENTICATION_MSG)
    ...
    def key_changed(self, key_name: str, old_keys: SecretKeyPairValues, new_keys: SecretKeyPairValues) -> None:
        try:
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    KEY_CHANGED_DEBUG_FORMAT.format(key_name, str(SecretKeyPair(old_keys)),
                                                    str(SecretKeyPair(new_keys))))
            else:
                logger.info(KEY_CHANGED_INFO_FORMAT.format(key_name))
            self._validate_key_name(key_name)
            key_changed_context: KeyChangedContext = self._create_context(self._keys_definitions[key_name], old_keys, new_keys)
    ...
    @staticmethod
    def _on_halt_strategy(key_name: str, callback_name: str, key_changed_context: KeyChangedContext):
        logger.error(
            ON_HALT_LOG_FORMAT.format(key_name=key_name, callback_name=callback_name, error=key_changed_context[callback_name]['error']))
        raise DoNothing()

    @staticmethod
    def _on_skip_strategy(key_name: str, callback_name: str, key_changed_context: KeyChangedContext):
        logger.error(
            ON_SKIP_LOG_FORMAT.format(
                key_name=key_name, callback_name=callback_name, error=key_changed_context[callback_name]['error']))

    @staticmethod
    def _on_skip_and_raise_strategy(key_name: str, callback_name: str, key_changed_context: KeyChangedContext):
        pass

    @staticmethod
    def _on_raise_strategy(key_name: str, callback_name: str, key_changed_context: KeyChangedContext):
        raise KeyChangedError(key_name, key_changed_context)

    def _create_context(self, key_definition: SecretKeyDefinition, old_keys: SecretKeyPairValues, new_keys: SecretKeyPairValues) -> KeyChangedContext:
        return KeyChangedContext(key_definition, self._callbacks_executions_error_handling[key_definition.on_key_changed_callback_error_strategy], old_keys, new_keys)
    ...
    def register_on_change(self, key_name: str, on_change_func: KeyChangedCallback, callback_name: str = None) -> None:
        ...
        callback_name = callback_name if callback_name is not None else DEFAULT_CALLBACK_NAME_FORMAT.format(key_name,
                                                                                 str(len(callbacks) + 1))
