...
from .key_definition import BaseSecretKeyDefinition, SecretKeyDefinition, SecretKeyDefinitionInitError
...
class SecretKey:
    ...
    def censor_key(self):
        ...
        return str_to_censor[0: revealedPartSize] + "*" * (key_length - 2 * revealedPartSize) + str_to_censor[
                                                                                               key_length - revealedPartSize:]


class SecretKeyInitError(InitError):
    def __init__(self, secret_key_value: Any) -> None:
        super().__init__('SecretKey', "secret_key_value type is %s" % str(type(secret_key_value)))
...
class SecretKeyPairInitError(InitError):
    def __init__(self, secret_key_pair_values: Any) -> None:
        super().__init__('SecretKeyPair', "secret_key_pair_values type is %s" % str(type(secret_key_pair_values)))
