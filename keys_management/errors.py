...
FETCH_AND_SET_ERROR_MSG_FORMAT = 'Failed to fetch and set key state for {key_name} from_repo: {why}'
...
class GetKeyError(KeysManagementError):
    def __init__(self, key_name: str, reason: str = None) -> None:
        super().__init__(GET_KEY_ERROR_MSG_FORMAT.format(key_name=key_name, reason=reason if reason else ''))


class KeyChangedError(KeysManagementError):
    def __init__(self, key_name, key_changed_context: KeyChangedContext = None) -> None:
...
class FetchAndSetStateFromRepoError(KeysManagementError):
    def __init__(self, key_name: str, why: str = None) -> None:
        super().__init__(FETCH_AND_SET_ERROR_MSG_FORMAT.format(key_name=key_name,
                                                        why=why if why else ""))
