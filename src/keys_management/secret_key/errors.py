...
from .consts import SECRET_KEY_TYPE_NAME, SECRET_KEY_PAIR_TYPE_NAME, SECRET_KEY_DEFINITION_TYPE_NAME
...
class SecretKeyInitError(InitError):
    def __init__(self, secret_key_value: Any) -> None:
        super().__init__(SECRET_KEY_TYPE_NAME, SECRET_KEY_ERR_MSG_FRMT % str(type(secret_key_value)))


class SecretKeyPairInitError(InitError):
    def __init__(self, secret_key_pair_values: Any) -> None:
        super().__init__(SECRET_KEY_PAIR_TYPE_NAME, PAIR_INIT_ERR_MSG_FRMT % str(type(secret_key_pair_values)))
