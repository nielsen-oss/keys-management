...
class TestDecryptedState:
    ...
    def test_on_enter__with_symmetric_key_store_keys_were_set(self, decrypted_state, symmetry_key_store):
        ...
        assert decrypted_state._secret_key_pair.encrypt_key.get_value() == expected_key
        assert decrypted_state._secret_key_pair.decrypt_key.get_value() == expected_key
    ...
    def test_on_enter__with_asymmetric_key_store_keys_were_set(self, decrypted_state, asymmetric_key_store):
        ...
        decrypted_state.enter()
        assert decrypted_state._secret_key_pair.encrypt_key.get_value() == expected_keys[ENCRYPTION_KEY_TYPE]
        assert decrypted_state._secret_key_pair.decrypt_key.get_value() == expected_keys[DECRYPTION_KEY_TYPE]
        assert decrypted_state._is_entered is True
    ...
    def test_opposite_state(self, decrypted_state):
        ...
        assert decrypted_state.get_opposite_state().get_opposite_state() == decrypted_state

    def test_set_keys_store(self, decrypted_state, symmetry_key_store):
        decrypted_state.set_keys_store(symmetry_key_store)
        assert decrypted_state._keys_store is not None
        assert symmetry_key_store() == decrypted_state._keys_store().encrypt_key.get_value()
