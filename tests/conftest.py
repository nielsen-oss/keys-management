...
def keys_management(empty_keys_management: KeysManagement, all_key_definitions: List[KeyDefForTest]):
    for key_def in all_key_definitions:
        empty_keys_management.define_key(key_def.name, key_def.keys_store, key_def.is_stateless(), key_def.use_case, key_def.is_target_data_accessible(), key_def.is_keep_in_cache())
...
def DE_stated_not_accessible(key_definition_factory) -> KeyDefForTest:
    ...
    return key_definition_factory(name=key_name, keys=convert_key_name_to_values(key_name),
                                  use_case=SecretKeyUseCase.ENCRYPTION_DECRYPTION, stateless=False, target_data_accessible=False, keep_in_cache=True)
...
def DE_stated_accessible(key_definition_factory):
    ...
    return key_definition_factory(name=key_name, keys=convert_key_name_to_values(key_name),
                                  use_case=SecretKeyUseCase.ENCRYPTION_DECRYPTION, stateless=False, target_data_accessible=True, keep_in_cache=True)
...
def DE_stateless_not_accessible(key_definition_factory):
    ...
    return key_definition_factory(name=key_name, keys=convert_key_name_to_values(key_name),
                                  use_case=SecretKeyUseCase.ENCRYPTION_DECRYPTION, stateless=True, target_data_accessible=False, keep_in_cache=True)
...
def DE_not_cached_accessible(key_definition_factory):
    ...
    return key_definition_factory(name=key_name, keys=convert_key_name_to_values(key_name),
                                  use_case=SecretKeyUseCase.ENCRYPTION_DECRYPTION, stateless=True, target_data_accessible=True, keep_in_cache=False)
...
def A_stated_not_accessible(key_definition_factory):
    ...
    return key_definition_factory(name=key_name, keys=key_val, use_case=SecretKeyUseCase.AUTHENTICATION, stateless=False,
...
def A_stated_accessible(key_definition_factory):
    ...
    return key_definition_factory(name=key_name, keys=key_val,
                                  use_case=SecretKeyUseCase.AUTHENTICATION, stateless=False, target_data_accessible=True, keep_in_cache=True, key_as_single=True)
...
def A_stateless_not_accessible(key_definition_factory):
    ...
    key_val = convert_key_name_to_value(key_name)
    return key_definition_factory(name=key_name, keys=key_val,
                                  use_case=SecretKeyUseCase.AUTHENTICATION,stateless=True, target_data_accessible=False, keep_in_cache=True, key_as_single=True)
...
def A_stateless_accessible(key_definition_factory):
    ...
    return KeyDefForTest(name=key_name, keys=key_val,
                         use_case=SecretKeyUseCase.AUTHENTICATION, stateless=True, target_data_accessible=True, keep_in_cache=True, key_as_single=True)
...
def stated_key_def(stated_key_name, stated_key) -> KeyDefForTest:
    return KeyDefForTest(name=stated_key_name, keys=stated_key, use_case=SecretKeyUseCase.ENCRYPTION_DECRYPTION, stateless=False)
