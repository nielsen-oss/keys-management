...
from keys_management import KeysManagement, StateRepoInterface, CryptoTool, OnKeyChangedCallbackErrorStrategy
...
@fixture
def keys_management(empty_keys_management: KeysManagement, all_key_definitions: List[KeyDefForTest]):
...
def key_def(key_name: str, stated_key: str) -> KeyDefForTest:
    return KeyDefForTest(name=key_name, keys=stated_key, use_case=SecretKeyUseCase.ENCRYPTION_DECRYPTION)
...
def DE_stated_not_accessible(key_definition_factory) -> KeyDefForTest:
    ...
    return key_definition_factory(name=key_name, keys=convert_key_name_to_values(key_name),
                                  use_case=SecretKeyUseCase.ENCRYPTION_DECRYPTION, stateless=False,
                                  target_data_accessible=False, keep_in_cache=True)
...
def DE_stated_accessible(key_definition_factory):
    ...
    return key_definition_factory(name=key_name, keys=convert_key_name_to_values(key_name),
                                  use_case=SecretKeyUseCase.ENCRYPTION_DECRYPTION, stateless=False,
                                  target_data_accessible=True, keep_in_cache=True)
...
def DE_stateless_not_accessible(key_definition_factory):
    ...
    return key_definition_factory(name=key_name, keys=convert_key_name_to_values(key_name),
                                  use_case=SecretKeyUseCase.ENCRYPTION_DECRYPTION, stateless=True,
                                  target_data_accessible=False, keep_in_cache=True)
...
def DE_not_cached_accessible(key_definition_factory):
    ...
    return key_definition_factory(name=key_name, keys=convert_key_name_to_values(key_name),
                                  use_case=SecretKeyUseCase.ENCRYPTION_DECRYPTION, stateless=True,
                                  target_data_accessible=True, keep_in_cache=False)
...
def A_stated_not_accessible(key_definition_factory):
    ...
    return key_definition_factory(name=key_name, keys=key_val, use_case=SecretKeyUseCase.AUTHENTICATION,
                                  stateless=False,
                                  target_data_accessible=False, keep_in_cache=True, key_as_single=True)
...
def A_stated_accessible(key_definition_factory):
    ...
    key_val = convert_key_name_to_value(key_name)
    return key_definition_factory(name=key_name, keys=key_val,
                                  use_case=SecretKeyUseCase.AUTHENTICATION, stateless=False,
                                  target_data_accessible=True, keep_in_cache=True, key_as_single=True)
...
def A_stateless_not_accessible(key_definition_factory):
    ...
    key_val = convert_key_name_to_value(key_name)
    return key_definition_factory(name=key_name, keys=key_val,
                                  use_case=SecretKeyUseCase.AUTHENTICATION, stateless=True,
                                  target_data_accessible=False, keep_in_cache=True, key_as_single=True)
...
def A_stateless_accessible(key_definition_factory):
    ...
    key_val = convert_key_name_to_value(key_name)
    return KeyDefForTest(name=key_name, keys=key_val,
                         use_case=SecretKeyUseCase.AUTHENTICATION, stateless=True, target_data_accessible=True,
                         keep_in_cache=True, key_as_single=True)


@fixture
def stateless_key_def(stateless_key_name: str, stateless_key: str) -> KeyDefForTest:
    return KeyDefForTest(name=stateless_key_name, keys=stateless_key,
                         use_case=SecretKeyUseCase.ENCRYPTION_DECRYPTION, stateless=True)


@fixture
def stated_key_def(stated_key_name, stated_key) -> KeyDefForTest:
    return KeyDefForTest(name=stated_key_name, keys=stated_key, use_case=SecretKeyUseCase.ENCRYPTION_DECRYPTION,
                         stateless=False)


@fixture
def halt_error_strategy_key_def(stateless_key_name: str, stateless_key: str):
    name = 'HALT_KEY_DEF'
    return KeyDefForTest(name=name, keys=stated_key, use_case=SecretKeyUseCase.ENCRYPTION_DECRYPTION, stateless=False,
                         on_key_changed_callback_error_strategy=OnKeyChangedCallbackErrorStrategy.HALT)


@fixture
def skip_error_strategy_key_def(stateless_key_name: str, stateless_key: str):
    name = 'SKIP_KEY_DEF'
    return KeyDefForTest(name=name, keys=stated_key, use_case=SecretKeyUseCase.ENCRYPTION_DECRYPTION, stateless=False,
...
@fixture
def skip_raise_error_strategy_key_def(stateless_key_name: str, stateless_key: str):
...
@fixture
def raise_error_strategy_key_def(stateless_key_name: str, stateless_key: str):
    name = 'RAISE_KEY_DEF'
    return KeyDefForTest(name=name, keys=stated_key, use_case=SecretKeyUseCase.ENCRYPTION_DECRYPTION, stateless=False,
...
@fixture
def all_key_definitions(DE_stated_not_accessible,
